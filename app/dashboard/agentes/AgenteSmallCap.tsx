...
export default function AgenteSmallCap({ isAdmin }: { isAdmin: boolean }) {
  ...
  async function runAgenteSmallCap() {
    ...
    try {
      ...
      addLog(`📝 Generando ${paso4Pass.length} informe(s) en RECOMENDACIONES AGENTE SMALL...`)
      ...
      addLog(`🎉 Agente Small completado: ${created} recomendación(es) en SMALL CAPS.`)
    ...
    <div className="rounded-xl border overflow-hidden" style={{ borderColor: 'rgba(201,168,76,0.25)' }}>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex gap-2">
            {phase === 'idle' || phase === 'done' || phase === 'error' ? (
              <button onClick={phase === 'idle' ? () => { setStepsOpen(true); runAgenteSmallCap() } : resetAgent} disabled={!isAdmin}
      ...
      {stepsOpen && <div className="px-6 py-5 space-y-3" style={{ borderTop: '1px solid rgba(201,168,76,0.12)' }}>
        {[
          ...
          { num: 5, label: 'GENERANDO INFORMES',      desc: 'Crea informe en RECOMENDACIONES AGENTE SMALL',         phaseVal: step5Phase },
        ].map(step => (
      ...
      {summary && (
        <div className="mx-6 mb-5 p-4 rounded-xl border" style={{ borderColor: 'rgba(201,168,76,0.3)', background: 'rgba(201,168,76,0.05)' }}>
          ...
          {summary.created > 0 && (
            <a href="/dashboard/acciones" className="text-[10px] font-mono underline mt-1 block" style={{ color: 'var(--gold)' }}>Ver en RECOMENDACIONES AGENTE SMALL →</a>
          )}
        </div>
      )}
