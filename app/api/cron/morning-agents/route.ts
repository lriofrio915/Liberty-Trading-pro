...
async function runPeterAgent(lynch: CachedLynchEntry[], today0: Date): Promise<MorningAgentResult> {
  ...
  await prisma.opportunity.deleteMany({ where: { category: 'PETER_LYNCH', active: true, publishedAt: { gte: today0 } } })
    ...
    try {
      await prisma.opportunity.create({
        data: {
          ...
          aiReport: null, minPlan: 'CLUB', category: 'PETER_LYNCH', active: true, status: 'COMPRAR',
        },
      })
...
export async function POST(req: NextRequest) {
  ...
  void (async () => {
    try {
      ...
      const [peter] = await Promise.allSettled([
