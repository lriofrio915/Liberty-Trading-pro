...
import { notifyDivisasSesgo } from '@/lib/notify-nexus'
...
export async function POST(req: NextRequest) {
  ...
  try {
    // Run full MAIA analysis (6 agents via OpenRouter) and extract divisas
    const result = await runFullAnalysis()
    const allActivos = result.activos.filter(a => a.sector === 'Divisas')
    ...
    const divisasTickers = allActivos.map(a => a.simbolo)

    const [prevLogs, morningLogs] = await Promise.all([
      prisma.sesgoIntradayLog.findMany({
        where: { simbolo: { in: divisasTickers }, timestamp: { gte: prevCutoff, lt: prevFloor } },
        orderBy: { timestamp: 'desc' },
      }),
      prisma.sesgoIntradayLog.findMany({
        where: { simbolo: { in: divisasTickers }, timestamp: { gte: today0 } },
    ...
    // Morning baseline: first snapshot per pair today
    ...
    void notifyDivisasSesgo(allActivos, changed, morningMap, prevMap, recomendaciones, isPreApertura)
    ...
    console.error('[cron/divisas-sesgo]', err)
