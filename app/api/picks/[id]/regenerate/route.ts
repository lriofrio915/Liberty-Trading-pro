...
// ── POST /api/picks/[id]/regenerate — regenerate AI report for existing pick ──
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // ── 1. Fetch fresh Yahoo Finance data ──
    const yf = await fetchTickerFinancials(ticker)
...

{
  ...
  "mes_año": "${new Date().toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })}"
      ...
      console.error('[regenerate] AI report generation failed:', aiErr)
    ...
    console.error('POST /api/picks/[id]/regenerate:', err)
