...
<p>Este club no te enseña a adivinar el mercado. Te enseña a construir un <strong>sistema</strong>: estrategias que se programan, se validan con estadística y se operan con reglas fijas, medidas con un track record propio.</p>
<h3>Los tres pilares</h3>
<ol>
  <li><strong>Video clases</strong> — la infraestructura (Claude Code, tu app de track record) y el método para crear estrategias con Claude Code, NinjaTrader 8 y Obsidian.</li>
...
<h3>Tu ruta</h3>
<ul>
  <li><strong>Día 1:</strong> activas tu cuenta fondeada e instalas NinjaTrader 8.</li>
  <li><strong>Semana 1:</strong> instalas el portafolio comunitario.</li>
  <li><strong>Semanas 2–8:</strong> recorres los módulos y construyes tu laboratorio.</li>
  <li><strong>Proyecto final:</strong> creas tu propia estrategia, la validas y la compartes con la comunidad.</li>
</ul>
...
<h3>1. Una máquina que no se apague</h3>
<p>NinjaTrader 8 solo corre en <strong>Windows</strong>. Tienes dos opciones:</p>
<ul>
  <li><strong>Tu PC</strong> — válido para aprender y hacer backtests. Para operar bots en real, la PC debe quedar encendida y conectada durante la sesión.</li>
  <li><strong>Un VPS con Windows</strong> — un servidor en la nube que corre 24/7. Es lo recomendado cuando los bots operan capital: no depende de tu internet ni de tu luz.</li>
</ul>
<p>Referencia de recursos: 4 GB de RAM como mínimo (8 GB recomendado), 2 núcleos y disco SSD.</p>
<h3>2. Tu suscripción a Claude</h3>
<p>El club te enseña a usar <strong>Claude Code</strong>, pero la suscripción es tuya y no está incluida. El plan <strong>Pro ($20/mes)</strong> es suficiente para todo el curso.</p>
<h3>3. Cuentas que vas a crear</h3>
<ul>
  <li>NinjaTrader (gratis para simulación y backtest).</li>
  <li>GitHub (gratis) — para guardar tu código.</li>
  <li>Vercel (gratis) — para publicar tu app de track record.</li>
  <li>Obsidian (gratis) — tu laboratorio de investigación.</li>
</ul>
''',
    ),
    dict(
...
<p>Claude Code es un asistente de programación que trabaja en tu computadora: lee tus archivos, escribe código, lo ejecuta y corrige errores. Tú describes lo que quieres; él lo construye y tú revisas.</p>
<h3>Instalación</h3>
<ol>
  <li>Descarga Claude Code desde <strong>claude.com/claude-code</strong> (app de escritorio o terminal).</li>
  <li>Inicia sesión con la misma cuenta de tu suscripción Pro.</li>
  <li>Crea una carpeta de trabajo, por ejemplo <code>C:\\Trading\\liberty</code>, y abre Claude Code dentro de ella.</li>
</ol>
<h3>El archivo CLAUDE.md</h3>
<p>Es la memoria del proyecto. Claude lo lee al empezar cada sesión. Escribe ahí lo que nunca debe olvidar:</p>
<ul>
  <li>Qué instrumento operas (MNQ) y en qué sesión (RTH o ETH).</li>
  <li>Tus reglas innegociables: todo valor como parámetro, stop loss obligatorio, logs en cada entrada y salida.</li>
  <li>Dónde guardas las estrategias y los resultados.</li>
</ul>
<h3>Buenas prácticas</h3>
<ul>
  <li>Pide una cosa a la vez y revisa el resultado antes de la siguiente.</li>
  <li>Si algo no compila, pega el error completo: Claude lo corrige mejor con el mensaje exacto.</li>
  <li>Nunca aceptes un "funciona" sin verlo compilar y correr tú mismo.</li>
</ul>
''',
    ),
    dict(
...
<p>Una estrategia pasa por decenas de versiones. Sin control de versiones no sabes qué cambió entre el backtest que funcionaba y el que dejó de funcionar. Git guarda cada cambio con su fecha y su motivo.</p>
<h3>Lo esencial</h3>
<ol>
  <li>Crea una cuenta en <strong>github.com</strong>.</li>
  <li>Pide a Claude Code: <em>"inicializa un repositorio git en esta carpeta y súbelo a un repositorio privado de GitHub"</em>.</li>
  <li>Cada vez que cierres un cambio importante, pide un <strong>commit</strong> con un mensaje claro: <em>"RSI2: stop a 60 ticks, WFO ventana 3"</em>.</li>
</ol>
<h3>Qué va en el repositorio</h3>
<ul>
  <li>El código <code>.cs</code> de cada estrategia.</li>
  <li>Las exportaciones de resultados (CSV) y los registros de WFO.</li>
  <li>Tu app de track record (módulo 02).</li>
</ul>
<p>Qué <strong>no</strong> va: contraseñas, claves de API ni datos de tu cuenta del bróker.</p>
''',
    ),
    ...
    dict(
        ...
        contenido='''
<h2>Lo que no se mide, no se gestiona</h2>
<p>Un track record es el historial completo de tus operaciones: ganadoras y perdedoras, sin recortes. Es la única forma de saber si tu sistema en real se parece a su backtest.</p>
<h3>Qué te responde</h3>
<ul>
  <li>¿El profit factor en real se parece al del Walk-Forward?</li>
  <li>¿El drawdown actual está dentro de lo esperado o es una señal de que el edge se rompió?</li>
  <li>¿Qué estrategia del portafolio aporta y cuál resta?</li>
</ul>
<h3>Las reglas de un track record honesto</h3>
<ol>
  <li>Se registran <strong>todas</strong> las operaciones, no solo las buenas.</li>
  <li>Se registran con comisiones reales.</li>
  <li>No se borra nada. Si hubo un error, se anota como error.</li>
</ol>
''' + NOTA.format('Publico mis pérdidas igual que mis ganancias. No vendo sueños — muestro datos.'),
    ),
    dict(
        modulo=1,
        ...
        contenido='''
<h2>De la idea a la app en una tarde</h2>
<p>Vas a construir una aplicación web propia (Next.js) con una base de datos donde viven tus operaciones.</p>
<h3>El pedido a Claude Code</h3>
<p>Empieza con una descripción clara, por ejemplo:</p>
<p><em>"Crea una app Next.js con una base de datos Supabase para registrar operaciones de trading. Cada operación tiene: fecha, estrategia, instrumento, dirección, contratos, precio de entrada, precio de salida, comisiones y P&amp;L neto. Quiero una página para registrar y otra para ver el historial."</em></p>
<h3>Cómo avanzar</h3>
<ol>
  <li>Pide primero la estructura y la base de datos. Revisa que los campos sean los correctos.</li>
  <li>Después, el formulario de registro. Pruébalo con una operación real.</li>
  <li>Después, el historial. Verifica que los totales cuadren con tu bróker.</li>
</ol>
<p>Cada paso termina con un commit (módulo 01).</p>
''',
    ),
    dict(
        modulo=1,
        ...
        contenido='''
<h2>Tu track record, en tu propio dominio</h2>
<ol>
  <li>Crea una cuenta en <strong>vercel.com</strong> usando tu GitHub.</li>
  <li>Importa el repositorio de tu app.</li>
  <li>Configura las variables de entorno (las claves de la base de datos). Nunca las subas al repositorio.</li>
  <li>Pulsa Deploy. En un par de minutos tienes una URL pública.</li>
</ol>
<h3>Cómo se actualiza</h3>
<p>Cada vez que haces push a la rama principal, Vercel publica la nueva versión. Por eso: prueba antes de hacer push.</p>
<h3>Opcional: tu dominio</h3>
<p>Puedes conectar un dominio propio desde el panel de Vercel. Un track record en tu dominio transmite profesionalismo.</p>
''',
    ),
    dict(
        modulo=1,
        ...
        contenido='''
<h2>Del listado al tablero</h2>
<p>Un historial de operaciones no dice nada por sí solo. Pide a Claude Code que calcule y grafique:</p>
<ul>
  <li><strong>Beneficio neto</strong> total y por estrategia.</li>
  <li><strong>Drawdown máximo</strong> — la mayor caída desde un pico. Es la métrica clave.</li>
  <li><strong>Profit factor</strong> — ganancias brutas ÷ pérdidas brutas.</li>
  <li><strong>Win rate</strong> — siempre junto al ratio riesgo/beneficio.</li>
  <li><strong>Curva de capital</strong> acumulada, total y por estrategia.</li>
</ul>
<h3>Compara real vs backtest</h3>
<p>Guarda también las métricas del Walk-Forward de cada estrategia y muéstralas al lado de las reales. Si el drawdown real supera con claridad el peor caso de Montecarlo, esa estrategia se revisa.</p>
''',
    ),
...
<ul>
  <li>Compara las operaciones del día con lo que la estrategia debía hacer.</li>
  <li>Registra todo en tu app de track record.</li>
